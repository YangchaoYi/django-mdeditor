...
// This wrapper function is to find the proper way to export the factory object.
(function (factory) {
  "use strict";
  ...
  // the entire function is to construct a editormd object.
  ...
  // editormd is a function object. Once called, it'll return an initialized
  ...
  // Specifies some default option values.
  editormd.defaults = {
  ...
  editormd.prototype = editormd.fn = {
              ...
              if (typeof marked !== "undefined") {
                const plantumlBlock = {
                  extensions: [{
                    ...
                    renderer(token) {
                      if (token.type === 'plantumlBlock') {
                        // Return markdown image format doesn't work because
                        // other renders won't parse the result anymore. 
                        // return "![](" + token.html + ")";
                        return token.html;
                      }
                    }
                  ...
                  async walkTokens(token) {
                    if (token.type === 'plantumlBlock') {
                      var url = "https://www.plantuml.com/plantuml/svg/" +
                                encode64(deflate(token.content, 9));
                      const res = await fetch(url);
                      token.html = await res.text();
                      // token.html = `<img src=${url} alt=${url}>`;
                    }
                  }
                };
    ...
    save: async function () {
      ...
      // cmValue = cmValue.replace(/@startuml((.|\s)*?)@enduml/g, function ($1) {
      //   var url =
      //     "https://www.plantuml.com/plantuml/img/" + encode64(deflate($1, 9));
      //   // console.info(url);
      //   return "![](" + url + ")";
      // });

      editormd.$marked.setOptions(markedOptions);
      var newMarkdownDoc = await editormd.$marked.parse(cmValue);
  ...
  editormd.markdownToHTML = async function (id, options) {
    var defaults = {
      gfm: true,
      toc: true,
      tocm: false,
    ...
    marked.setOptions(markedOptions);
    var markdownParsed = await marked.parse(cmValue);
